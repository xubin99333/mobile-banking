...
export function callRPC (
  ...
  Vue.prototype.$goose.tools.getAPPDataCache({
    ...
    if (config.method == 'post' || config.method === 'put') {
      requestParams[0]._requestBody = { reqMsg: extData.params }
    } else {
      requestParams = [extData.params]
    }
    ...
    Vue.prototype.$goose.tools.rpc(Object.assign(reqParams, { goosePlatformExtData: extData }),
      (rspData, rpcData) => {
        ...
        console.log(`接口 ${operationType} 的返回参数: ${JSON.stringify(rspData)}`)
        try {
          if (config.showLoading) {
            Toast.clear()
          }
          ...
          // response.body.header.code == 200
          // if (response.body.header) {

          // }
          if (response.header &&
              response.header['Result-Status'] == 1000 &&
              response.header['Business-Status'] == 200
          ) {
            if (response.body && response.body.header && response.body.header.code == 50002) {
              let errEvent = new CustomEvent(`rpcResult-${rpcExtData.operationType}`, {
                detail: {
                  type: 'failure',
                  param: response.body.header,
                  ext: rpcExtData
                }
              })
