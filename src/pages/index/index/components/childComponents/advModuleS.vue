      ...
      <div class="img_borrow" v-if="boothResource && boothResource.length > 0">
        <goose-swipe
         ...
         indicator-color="white">
          <goose-swipe-item v-for="(item,index) in boothResource" :key="index" @click="toAds(index)">
            <img :src="item" alt="" />
          </goose-swipe-item>
  ...
  methods: {
    ...
    getBoothFn () {
      ...
      for(let i = 0; i<this.boothListRes.length; i++) {
        if(this.boothListRes[i].boothType == 2) {
          res = this.boothListRes[i]
          console.log('~~~~~getBooth获取小广告~~~~~', res)
